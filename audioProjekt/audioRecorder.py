...
def sense(time):
    (nchannels, sampwidth, sampformat, framerate, length) = (1, 2, pa.SAMPLE_S16LE, 44100, time)
    samples = []
    ...
    with pa.simple.open(
        ...
        for i in range(len(trans)):
            if abs(trans[i]) > abs(trans[ans]):
                ans2 = ans
                ans = i
        return (int(ans / length), abs(trans[ans]) / abs(trans[ans2]))
